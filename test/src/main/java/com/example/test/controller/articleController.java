...
public class articleController {
    ...
    public String update(articleForm form){  //전체 값을 받아옴
        log.info(form.toString());
        ...
        //2. 엔티티를 DB로 저장한다.
        //2-1.DB에서 기존 데이터를 가져온다 (해당 아이디의 글만 가져옴)
        Article target = articleRepository.findById(articleEntity.getId()).orElse(null);
        //2-2. 기존 데이터에 값을 갱신한다.
        if(target!=null){  //해당 id의 데이터가 비어있지 않으면
    ...
    public String delete(@PathVariable Long id, RedirectAttributes rttr){
        ...
        if(target!=null){
            ...
            rttr.addFlashAttribute("msg","삭제가 완료되었습니다.");  //휘발성 데이터 넘기기
        }
